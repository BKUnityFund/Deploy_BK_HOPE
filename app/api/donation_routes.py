...
async def create_donation(
    ...
    current_numeric = crud_project.get_current_numeric_by_project(db=db, id_project=donation.id_project)
    current_numeric += donation.amount
    crud_project.update_current_numeric(db=db, id_project=donation.id_project, current_numeric=current_numeric)
    # 3. Gửi dữ liệu WebSocket (không block API)
    asyncio.create_task(
        manager.broadcast(
            project_id=donation.id_project,
            message={
                "project_id": donation.id_project,
